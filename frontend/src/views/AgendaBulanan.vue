    ...
    <Transition name="detail">
      <div v-if="detailViewItems" class="fixed inset-0 z-50 bg-black animate-fadeIn">
        <div v-if="detailViewItems.length > 1" class="absolute top-6 left-1/2 -translate-x-1/2 z-60 flex items-center gap-2 bg-black/50 backdrop-blur-sm rounded-full px-4 py-2 border border-white/10">
          <button v-for="(item, dIdx) in detailViewItems" :key="item.id"
                  ...
                  :class="[
                    ...
                    dIdx === detailActiveIdx ? 'bg-accent text-blue-950 shadow-lg' : 'text-white/70 hover:text-white hover:bg-white/10'
                  ]">
        ...
        <template v-if="currentDetailItem">
          <DetailNews v-if="currentDetailItem.category === 'Artikel'" :item="currentDetailItem" @close="closeDetailView" />
          <DetailImage v-else-if="currentDetailItem.category === 'Gambar'" :item="currentDetailItem" @close="closeDetailView" />
          <DetailVideo v-else-if="currentDetailItem.category === 'Video'" :item="currentDetailItem" @close="closeDetailView" />
        </template>
      </div>
    </Transition>
  </div>
...
function openSingleDetail(item) {
  detailViewItems.value = [transformItem(item)]
