<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <h2 class="text-lg font-bold" style="color: var(--text-heading)">{{ isEdit ? 'Edit Media' : 'Upload Media Baru' }}</h2>
    </div>

    <!-- ═══ FORM CARD ═══ -->
    <div class="form-card rounded-2xl p-6 flex flex-col gap-6">

      <!-- ── Row 1: Judul + Tipe + Status ── -->
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <textarea v-model="form.description" rows="4" class="filter-input rounded-xl py-2.5 px-4 text-sm focus:outline-none focus:ring-1 focus:ring-accent resize-none" placeholder="Deskripsi media..."></textarea>
      </div>

      <!-- ── Image Upload ── -->
      <div class="flex flex-col gap-1.5">
        <label class="text-sm font-medium" style="color: var(--text-body)">{{ form.category === 'Video' ? 'Thumbnail Video' : 'Upload Gambar *' }}</label>
      ...
      <!-- ── Video Upload (only for Video category) ── -->
      <div v-if="form.category === 'Video'" class="flex flex-col gap-1.5">
        ...
        <div class="upload-zone rounded-xl p-6 flex flex-col items-center gap-3 cursor-pointer transition-all" @click="$refs.videoInput.click()" @dragover.prevent="videoDragOver = true" @dragleave="videoDragOver = false" @drop.prevent="handleVideoDrop" :class="{ 'drag-over': videoDragOver }">
          <div v-if="videoPreview" class="relative w-full max-w-md">
            <video :src="videoPreview" class="w-full h-48 rounded-lg object-cover" controls @loadedmetadata="onVideoMeta"></video>
      ...
      <div class="flex justify-end gap-3 pt-2">
        <router-link to="/administrator/gallery-video" class="px-6 py-2.5 rounded-lg text-sm font-medium cursor-pointer" style="color: var(--text-body); border: 1px solid var(--border)">Batal</router-link>
...
const categoryOptions = [{ name: 'Gambar (Foto)', value: 'Gambar' }, { name: 'Video', value: 'Video' }]
...
function removeVideo() { videoFile.value = null; videoPreview.value = null; removeVideoFlag.value = true; form.value.duration = null }

function onVideoMeta(e) {
  const dur = Math.round(e.target.duration)
  if (dur && dur > 0) form.value.duration = dur
}
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) await galleryStore.updateGallery(route.params.id, fd)
    else await galleryStore.createGallery(fd)
    router.push({ name: 'AdminGalleryVideo' })
  } catch (e) {
