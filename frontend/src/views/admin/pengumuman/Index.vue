<template>
  <div class="flex flex-col gap-6">
    <div class="flex items-center justify-between">
      <router-link :to="{ name: 'AdminPengumumanCreate' }" class="flex items-center gap-2 rounded-lg h-10 px-5 bg-accent text-btn-text font-bold transition-colors hover:bg-accent/90 shadow-[0_0_15px_rgba(251,191,36,0.3)] shrink-0 cursor-pointer active:scale-95" style="color: var(--text-btn)">
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="table-body">
            <tr v-if="items.length === 0">
              <td colspan="8" class="px-4 py-12 text-center text-sm" style="color: var(--text-muted)">Tidak ada data pengumuman</td>
            </tr>
            <tr v-for="(item, idx) in items" :key="item.id" class="table-row-hover">
              ...
              <td class="px-4 py-4 text-right">
                <div class="flex items-center justify-end gap-1">
                  <router-link :to="{ name: 'AdminPengumumanEdit', params: { id: item.id } }" class="action-btn p-2 rounded-lg cursor-pointer" title="Edit"><span class="material-symbols-outlined text-[20px] text-accent">edit</span></router-link>
...
async function deleteItem() {
  deleting.value = true
  try {
    await api.delete(`/announcements/${deletingItem.value.id}`)
