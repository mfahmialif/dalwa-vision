<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <h2 class="text-lg font-bold" style="color: var(--text-heading)">{{ isEdit ? 'Edit Konten' : 'Tambah Konten' }}</h2>
    </div>
    ...
    <div v-else class="form-card rounded-2xl p-6 flex flex-col gap-6">
      ...
      <div class="flex justify-end gap-3 pt-2">
        <router-link to="/administrator/info-terkini" class="px-6 py-2.5 rounded-lg text-sm font-medium cursor-pointer" style="color: var(--text-body); border: 1px solid var(--border)">Batal</router-link>
...
async function handleSubmit() {
  ...
  try {
    ...
    router.push({ name: 'AdminInfoTerkini' })
  } catch (e) {
