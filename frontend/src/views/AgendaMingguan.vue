<template>
  <div class="relative w-screen h-screen overflow-hidden text-white font-display antialiased">
    ...
    <div class="relative z-10 flex flex-col h-full p-4 md:p-8 gap-4 md:gap-5 max-w-[1920px] mx-auto">
      ...
      <!-- ═══════ WEEKLY CARDS ═══════ -->
      <main class="flex-1 flex items-center justify-center w-full overflow-x-auto portrait:overflow-y-auto py-2">
        <div class="flex portrait:flex-wrap items-center justify-center gap-2 md:gap-3 w-full h-full min-w-0">
          <div v-for="(day, index) in weekDays" :key="day.name"
            ...
            <!-- Agenda Items -->
            <div :class="['flex flex-col overflow-hidden', index === selectedDayIndex ? 'gap-4 flex-1 overflow-y-auto' : 'gap-3']">
    ...
    <!-- ═══════ DETAIL OVERLAYS (Multi-item) ═══════ -->
    <Transition name="detail">
      <div v-if="detailViewItems" class="fixed inset-0 z-50 bg-black animate-fadeIn">
        <!-- Navigation tabs (if multi) -->
        <div v-if="detailViewItems.length > 1" class="absolute top-6 left-1/2 -translate-x-1/2 z-60 flex items-center gap-2 bg-black/50 backdrop-blur-sm rounded-full px-4 py-2 border border-white/10">
          <button v-for="(item, dIdx) in detailViewItems" :key="item.id"
                  @click="detailActiveIdx = dIdx"
                  :class="[
                    'flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-bold transition-all cursor-pointer',
                    dIdx === detailActiveIdx
                      ? 'bg-accent text-blue-950 shadow-lg'
                      : 'text-white/70 hover:text-white hover:bg-white/10'
                  ]">
            <span class="material-symbols-outlined text-[16px]">{{ item.icon || 'event' }}</span>
            {{ item.title.length > 20 ? item.title.substring(0, 20) + '…' : item.title }}
          </button>
        </div>

        <!-- Detail content based on current detail item -->
        <template v-if="currentDetailItem">
          <DetailNews v-if="currentDetailItem.category === 'Artikel'"
                      :item="currentDetailItem"
                      @close="closeDetailView" />
          <DetailImage v-else-if="currentDetailItem.category === 'Gambar'"
                       :item="currentDetailItem"
                       @close="closeDetailView" />
          <DetailVideo v-else-if="currentDetailItem.category === 'Video'"
                       :item="currentDetailItem"
                       @close="closeDetailView" />
        </template>
      </div>
...
// ── Detail View (Multi-item, like AgendaHarian) ──
const detailViewItems = ref(null)
const detailActiveIdx = ref(0)

const currentDetailItem = computed(() => {
  if (!detailViewItems.value || detailViewItems.value.length === 0) return null
  return detailViewItems.value[detailActiveIdx.value] || detailViewItems.value[0]
})

function openGroupDetail(group) {
  detailViewItems.value = group.items.map(transformItem)
  detailActiveIdx.value = 0
}

function closeDetailView() {
  detailViewItems.value = null
  detailActiveIdx.value = 0
}
