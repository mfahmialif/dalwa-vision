<template>
  <div class="relative w-screen h-screen overflow-hidden font-display text-slate-100 selection:bg-accent selection:text-[#0a192f]">
    ...
    <Transition name="detail">
      <div v-if="selectedItem" class="fixed inset-0 z-50 bg-black/95 flex items-center justify-center animate-fadeIn p-4" @click.self="closeDetail">
        <div class="relative max-w-4xl w-full max-h-[90vh] overflow-y-auto bg-[#0a192f] border border-accent/20 rounded-2xl md:rounded-3xl p-6 md:p-10 shadow-[0_0_60px_rgba(0,0,0,0.8)]">
          <!-- Priority Badge -->
          <div class="flex items-center justify-between mb-6">
            <span :class="priorityBadge(selectedItem.priority)" class="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border">
              <span class="material-symbols-outlined text-sm!">{{ priorityIcon(selectedItem.priority) }}</span>
              {{ selectedItem.priority }}
            </span>
            <button @click="closeDetail" class="text-slate-400 hover:text-accent transition-colors cursor-pointer">
              <span class="material-symbols-outlined text-2xl">close</span>
            </button>
          </div>

          <!-- Title -->
          <h1 class="text-3xl lg:text-4xl font-black text-white mb-4 leading-tight">{{ selectedItem.title }}</h1>
          ...
          <div class="flex flex-wrap items-center gap-4 mb-8 text-sm text-slate-400">
            ...
            <span v-if="selectedItem.audience" class="flex items-center gap-1.5">
              <span class="material-symbols-outlined text-accent text-base!">groups</span>
              {{ selectedItem.audience }}
            </span>
            <span v-if="selectedItem.location" class="flex items-center gap-1.5">
          ...
          <div class="flex items-center justify-between mt-10 pt-6 border-t border-white/10">
            <button v-if="detailIndex > 0" @click="goDetailPrev"
                    class="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/20 hover:border-accent text-slate-300 hover:text-accent transition-all cursor-pointer">
              <span class="material-symbols-outlined">chevron_left</span>
              <span class="text-sm font-bold">Sebelumnya</span>
            </button>
            <span v-else></span>
            <span class="text-xs text-slate-500 font-bold tabular-nums">{{ detailIndex + 1 }} / {{ items.length }}</span>
            <button v-if="detailIndex < items.length - 1" @click="goDetailNext"
                    class="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/20 hover:border-accent text-slate-300 hover:text-accent transition-all cursor-pointer">
              <span class="text-sm font-bold">Selanjutnya</span>
...
// ── Detail ──
const selectedItem = ref(null); const detailIndex = ref(-1)
...
function goDetailPrev() {
  if (detailIndex.value > 0) { detailIndex.value--; selectedItem.value = items.value[detailIndex.value] }
}
function goDetailNext() {
  if (detailIndex.value < items.value.length - 1) { detailIndex.value++; selectedItem.value = items.value[detailIndex.value] }
}
function closeDetail() { selectedItem.value = null; detailIndex.value = -1 }
