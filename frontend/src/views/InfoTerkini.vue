<template>
  <div class="relative w-screen h-screen overflow-hidden font-display text-slate-100 selection:bg-accent selection:text-[#0a192f]">
      ...
      <header class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 md:mb-6 pb-3 md:pb-4 border-b border-accent/20 gap-3">
        <div class="flex items-center gap-3 md:gap-4">
          ...
          <div>
            ...
            <span class="text-xs md:text-sm font-bold text-accent uppercase tracking-widest">Info Terkini</span>
          </div>
        </div>
    ...
    <Transition name="detail">
      <DetailNews v-if="selectedItem && selectedItem.category === 'Artikel'"
                  :item="selectedItem" @close="closeDetail" />
    </Transition>
    <Transition name="detail">
      <DetailImage v-if="selectedItem && selectedItem.category === 'Gambar'"
                   :item="selectedItem" @close="closeDetail" />
    </Transition>
    <Transition name="detail">
      <DetailVideo v-if="selectedItem && selectedItem.category === 'Video'"
                   :item="selectedItem" @close="closeDetail" />
    </Transition>
  </div>
</template>
...
function openDetail(item) {
  selectedItem.value = {
    ...
    date: (() => { const d = new Date(item.created_at); const days = ['Minggu','Senin','Selasa','Rabu','Kamis','Jumat','Sabtu']; const months = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember']; return `${days[d.getDay()]}, ${d.getDate()} ${months[d.getMonth()]} ${d.getFullYear()}` })(),
