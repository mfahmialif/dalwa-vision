<template>
  <div class="relative w-screen h-screen overflow-hidden font-display text-slate-100 selection:bg-accent selection:text-[#0a192f]">
      ...
      <main class="flex-1 flex flex-col gap-5 overflow-hidden">
        ...
        <div class="flex items-end flex-col sm:flex-row justify-between px-1 md:px-2 gap-3">
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <div class="flex items-center gap-1 bg-[#050e1f]/60 border border-white/10 rounded-full px-3 py-1">
              ...
              <select v-model.number="perPage" @change="currentPage = 1; loadGalleries()" class="bg-transparent text-white text-xs font-bold outline-none cursor-pointer appearance-none px-1">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 portrait:lg:grid-cols-2 gap-4 md:gap-6 flex-1 pb-4 overflow-y-auto">
          ...
          <article v-if="galleryItems.length > 0"
                   @click="openDetail(galleryItems[0])"
                   class="group relative flex flex-col justify-end overflow-hidden rounded-xl bg-[#050e1f] border-4 border-accent shadow-[0_0_40px_rgba(251,191,36,0.5)] transition-transform duration-300 scale-[1.02] cursor-pointer min-h-[200px] md:min-h-0">
            ...
            <div class="relative z-10 p-6 flex flex-col gap-3">
              ...
              <h3 class="text-2xl font-bold text-white leading-tight line-clamp-2">{{ galleryItems[0].title }}</h3>
          ...
          <!-- Empty state -->
          <div v-if="galleryItems.length === 0" class="col-span-3 flex flex-col items-center justify-center gap-3 text-slate-500">
            <span class="material-symbols-outlined text-6xl">filter_none</span>
            <p class="text-lg font-medium">Tidak ada konten untuk filter ini</p>
          </div>
        </div>
      </main>
    ...
    <Transition name="detail">
      <DetailImage v-if="detailItem && detailItem.category === 'Gambar'"
                   :item="detailItem" :hasPrev="detailIndex > 0" :hasNext="detailIndex < galleryItems.length - 1"
                   @close="closeDetail" @prev="goDetailPrev" @next="goDetailNext" />
    </Transition>
    <Transition name="detail">
      <DetailVideo v-if="detailItem && detailItem.category === 'Video'"
                   :item="detailItem" :hasPrev="detailIndex > 0" :hasNext="detailIndex < galleryItems.length - 1"
                   @close="closeDetail" @prev="goDetailPrev" @next="goDetailNext" />
    </Transition>
  </div>
...
function openDetail(item) {
  const idx = galleryItems.value.findIndex(g => g.id === item.id)
  detailIndex.value = idx >= 0 ? idx : 0
  detailItem.value = buildDetailItem(item)
}

function goDetailPrev() {
  if (detailIndex.value > 0) {
    detailIndex.value--
    detailItem.value = buildDetailItem(galleryItems.value[detailIndex.value])
  }
}

function goDetailNext() {
  if (detailIndex.value < galleryItems.value.length - 1) {
    detailIndex.value++
    detailItem.value = buildDetailItem(galleryItems.value[detailIndex.value])
  }
}

function closeDetail() { detailItem.value = null; detailIndex.value = -1 }
